...
export const PersonalInfoForm = (props: iProps) => {
  ...
  const deleteItem = (item: number) => {
    let arr = values.category_ids?.filter(
      (ele) => ele.toString() !== item.toString()
    );
  ...
  useEffect(() => {
    ...
    if (
      ...
      (errors.phone_numbers as Array<any>).length > 0
    ) {
            ...
            <PhoneInput
              ...
              internationalCode={
                values.phone_numbers
                  ? values.phone_numbers[0].international_code
                  : ""
              }
              setValue={setValue as Function}
              phoneNumberError={
                errors.phone_numbers && (errors.phone_numbers as []).length > 0
                  ? (errors.phone_numbers as any[])[0].phone
                  : ""
              }
  ...
  return (
    <Row className="personalInfoContainer p-1">
      <Col
        sm={4}
        xs={12}
        className="d-flex d-sm-block justify-content-center p-2 p-sm-0"
      >
        <InputFile
          value={values.profile_picture}
          name="profile_picture"
      ...
      <Col sm={8} xs={12}>
        <Row className="gy-3">
          <Col xs={12}>
            <Row className="gy-2">
              ...
              <Col xs={12}>
                <PhoneInput
                  phone={
                    values.phone_numbers ? values.phone_numbers[0].phone : ""
                  }
                  internationalCode={
                    values.phone_numbers
                      ? values.phone_numbers[0].international_code
                  ...
                  phoneNumberError={
                    errors.phone_numbers &&
                    (errors.phone_numbers as []).length > 0
                      ? (errors.phone_numbers as any[])[0].phone
                      : ""
                  }
                  touched={touched.phone_numbers as boolean}
          ...
          <Col xs={12}>
            <Row className="gy-2">
              <Col xs={12}>
                <div className="text">{t("SocialMediaAccounts")}</div>
              </Col>
              <Col xs={12}>
                <InputWithIcon
                  label={t("Facebook")}
